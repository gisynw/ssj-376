...
----------------------------------------------
-- ST_Disjoint
-- Example 1: Find streets that do not intersect with any subway station with 50 meter buffer
SELECT ch05.streets.name
FROM ch05.streets
WHERE NOT EXISTS (
    SELECT 1 FROM ch05.subway
    WHERE ST_Disjoint(ch05.streets.geom, ch05.subway.geom) = FALSE
);
